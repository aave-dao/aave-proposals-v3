...
- The shared constructor helper deploys upstream `FixedPriceAdapter` instances for all 36 USD targets. V2 additionally uses upstream `CLSynchronicityPriceAdapterBaseToPeg` to divide the fixed USD target by the live canonical Chainlink ETH/USD feed and return 18-decimal ETH. V3 returns eight-decimal USD directly. Source files are copied unchanged from `aave-dao/aave-price-feeds` revision `00d0f14b0734dc6faf41960bb9023c0b742a944b` under `dependencies/`.
- Deployment remains in payload constructors; immutable feed addresses are safe under governance delegatecall. The upstream fixed feeds allow updates by pool admins through the corresponding market's V3 ACL manager (V3 Core Ethereum/Polygon ACL managers for V2 feeds). The conversion adapter returns zero for nonpositive source prices, allowing AaveOracle's existing fallback path, rather than reverting. No custom pricing contract remains.
...
- Updated suite: **32 tests passed, 0 failed, 0 skipped**, across 11 test contracts. This includes all 10 payload state transitions, generated execution/E2E tests, reserve-configuration checks and four upstream-adapter integration tests (including pool-admin price updates and invalid conversion inputs).
- A temporary fork probe measured approximately 9.43M gas for the largest constructor (V2 Ethereum payload plus 26 upstream adapters), below the repository's 15.94M buffered transaction threshold. This measures creation work, not a broadcast transaction.
