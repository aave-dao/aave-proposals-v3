...
contract AaveV3Ethereum_SafetyModuleAllowanceUpdate_20260901_Test is ProtocolV3TestBase {
  ...
  function test_stkAaveAllowanceUpdated() public {
    ...
    assertEq(
      allowanceAfter,
      expected,
      'stkAAVE allowance should fund a quarter of the backlog gap plus accrual and 90 days of emissions'
    );
    assertApproxEqAbs(
      allowanceAfter,
      30_612.44 ether,
      1 ether,
      'stkAAVE allowance should match the quarter-gap target at the fork block'
    );
  }
  ...
  function test_delayedExecutionStillCoversClaims() public {
    ...
    assertEq(
      ...
      expected,
      'stkAAVE allowance should fund a quarter of the backlog gap plus 90 days when executed late'
    );
